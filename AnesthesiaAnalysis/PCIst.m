...
function [pci,dNST,signal_svd,parameters]=PCIst(signal_evk,times,parameters)

if nargin<3
    parameters=[];
end
parameters=checkparameters(parameters);
%signal_evk= wdenoise(signal_evk')';
...
function [dNST,max_thresholds,D_base,D_resp]=statetransitions(signal,times,parameters)
    ...
    for i=1:size(signal,1)
        ...
        bufferAmp=std(reshape(D_base(i,:,:),[1 size(D_base,2)*size(D_base,3)]))*5;
        ...
        for j=1:size(thresholds,1)
            ...
            RP(D_resp(i,:,:)>=thresholds(j,i)+bufferAmp)=-1;
            RP(RP==0)=NaN;
            RP1=RP;
            RP2=RP;
            for counterRP1=2:size(RP,1)
                for counterRP2=2:size(RP,2)
                    if isnan(RP1(counterRP1,counterRP2))
                        RP1(counterRP1,counterRP2)=RP1(counterRP1-1,counterRP2);
                    end
                    
                    if isnan(RP2(counterRP1,counterRP2))
                        RP2(counterRP1,counterRP2)=RP2(counterRP1,counterRP2-1);
                    end
                end
            end
            RP=nanmax(RP1,RP2);
...
%ccplot([1:591],squeeze(D_resp(1,500,:))',squeeze(D_resp(1,500,:))',colormap(jet))
