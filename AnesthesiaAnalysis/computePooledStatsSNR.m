function pVals = computePooledStatsSNR(snrWakeEMU, snrSleep, snrWakeOR, snrAnesthesia, whatToCompare, titNameForPlot, cfgStats)

if ~exist(cfgStats.dirImages,'dir'), mkdir(cfgStats.dirImages); end
pVals=[];

%Put data together for the selected timeframe
% meanData = [snrWakeEMU.meanVariability.(whatToCompare),snrSleep.meanVariability.(whatToCompare),snrORWake.meanVariability.(whatToCompare),snrAnesthesia.meanVariability.(whatToCompare)];
% allChData = {snrWakeEMU.VariabilityPerCh.(whatToCompare),snrSleep.VariabilityPerCh.(whatToCompare),snrORWake.VariabilityPerCh.(whatToCompare),snrAnesthesia.VariabilityPerCh.(whatToCompare)};
%meanData = [snrWakeEMU.meanVariability.(whatToCompare),snrSleep.meanVariability.(whatToCompare),snrORWake.meanVariability.(whatToCompare),snrAnesthesia.meanVariability.(whatToCompare)];
allChData = {snrWakeEMU,snrSleep,snrWakeOR,snrAnesthesia};
meanData = allChData;
