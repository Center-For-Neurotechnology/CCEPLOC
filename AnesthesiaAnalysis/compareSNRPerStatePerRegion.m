function compareSNRPerStatePerRegion(pooledSNRFileName, dirResults, cfgStats)
...
diary([dirResults,filesep,'log','CompareSNRPerRegion',cfgStats.anatRegionFor,'.log'])


%% Load SNR Pooled file
stSNR = load(pooledSNRFileName);
nStates = length(stSNR.stateNames);

%% Get SNR and anatomical region for each patient and state
SNRPerState=cell(1,nStates);
normSNRPerState=cell(1,nStates);
...
for iState=1:nStates
    thisState = stSNR.stateNames{iState};
    SNRVals = stSNR.(strcat('snrVal',thisState));
    normSNRVals = SNRVals; % NOT really normalized stSNR.(strcat('meanStatsEEG',thisState)).normSNRPerCh.(whatIntervalToUse);
 %   nTrialPerCh = stSNR.(strcat('meanStatsEEG',thisState)).nTrialPerCh;
    SNRPerState{iState} =   SNRVals;
    normSNRPerState{iState} =   normSNRVals;
    nTrialPerChPerState{iState} =  zeros(1,length(SNRVals));
    stimSitesPerState{iState} = stSNR.stimPatChNames.(thisState);
    chNamesPNamesPerState{iState} = stSNR.bipChNames.(thisState);
    anatRegionsPerChPerState{iState} = stSNR.bipChAnatRegion.(thisState);
    anatRegionsStimChPerState{iState} = stSNR.stimChAnatRegion.(thisState);
    RASCoordPerChPerState{iState} = stSNR.bipChRASCoord.(thisState);
    RASCoordStimChPerState{iState} = stSNR.stimChRASCoord.(thisState);
    rechInStimShaftPerState{iState} = stSNR.rechInStimShaft.(thisState);
...
SNRPerStatePerRegion = cell(nStates,length(regionNames));
logSNRPerStatePerRegion = cell(nStates,length(regionNames));
normSNRPerStatePerRegion = cell(nStates,length(regionNames));
...
for iState=1:nStates
    [indChWithinStimRegion, SNRWithinStimRegion, stimChNamesPerRegion] = findChannelsWithinRegion(gralRegionsStimChPerState{iState}, regionNames, SNRPerState{iState}, stimSitesPerState{iState});
    ...
    for iRegion=1:length(regionNames)
        ...
        elseif strcmpi(cfgStats.anatRegionFor,'stimRespCh') %  both stim and rec Ch (within the same region)
            ...
            indCh = 1:length(SNRPerState{iState});
        end
        % Assign depending on what we want to analyse - log is now done
        % directly on full SNR measure - before mean across timepoints
        logSNRPerStatePerRegion{iState,iRegion} = log10(SNRPerState{iState}(indCh)); % USE log10 to make it more normal!!!
        SNRPerStatePerRegion{iState,iRegion} = SNRPerState{iState}(indCh);% keep also SNR values
        normSNRPerStatePerRegion{iState,iRegion} = normSNRPerState{iState}(indCh);% keep also SNR values
        nTrialPerChPerStatePerRegion{iState,iRegion} = nTrialPerChPerState{iState}(indCh);% keep also SNR values
        stimSitesPerStatePerRegion{iState,iRegion} = stimSitesPerState{iState}(indCh); % only stim channels in this region
        nPairsPerStatePerRegion(iState,iRegion) = length(indCh);%sum(~isnan(SNRWithinStimRegion{1}));
...
for iRegion=1:length(regionNames)
    ...
    cfgStats.ylabel = ['SNR ', regionNames{iRegion}] ;
    titName = [cfgStats.titName, ' ', regionNames{iRegion}] ;
    plotWakeVsAnesthesiaPerCh([], logSNRPerStatePerRegion(:,iRegion), titName, cfgStats, pairComps);
    
end

%% ANOVA
for iRegion=1:length(regionNames)
    allChData = [logSNRPerStatePerRegion(:,iRegion)];
...
relativeSNR = cell(size(pairComps,1),length(regionNames));
...
for iRegion=1:length(regionNames)
    ...
    for iComp=1:size(pairComps,1)
        ...
        varibilityThisRegionComp1 = logSNRPerStatePerRegion{pairComps(iComp,1),iRegion}(indIn1);
        varibilityThisRegionComp2 = logSNRPerStatePerRegion{pairComps(iComp,2),iRegion}(indIn2);
        if ~isempty(commonCh) && ~isempty(varibilityThisRegionComp1) && ~isempty(varibilityThisRegionComp2)
            [pairedTtest, medianVal1, medianVal2, testName] = computePairedTtestSaveInXls(varibilityThisRegionComp1, varibilityThisRegionComp2, titName,legLabel,xlsFileName,[cfgStats.sheetName,num2str([pairComps(iComp,:)])],cfgStats.dirImages,cfgStats.useParam);
            ...
            relativeSNR{iComp,iRegion} = (varibilityThisRegionComp1 - varibilityThisRegionComp2) ./(varibilityThisRegionComp1 + varibilityThisRegionComp2);
            ...
            statsResults.WithStimChInRegion.(regionNames{iRegion}).([cfgStats.legLabel{pairComps(iComp,:)}]).q025075Feat1 = [quantile(varibilityThisRegionComp1, 0.25) quantile(varibilityThisRegionComp1, 0.75)];
            statsResults.WithStimChInRegion.(regionNames{iRegion}).([cfgStats.legLabel{pairComps(iComp,:)}]).q025075Feat2 = [quantile(varibilityThisRegionComp2, 0.25) quantile(varibilityThisRegionComp2, 0.75)];
            statsResults.WithStimChInRegion.(regionNames{iRegion}).([cfgStats.legLabel{pairComps(iComp,:)}]).relativeVar = relativeSNR{iComp,iRegion};
            statsResults.WithStimChInRegion.(regionNames{iRegion}).([cfgStats.legLabel{pairComps(iComp,:)}]).commonCh =commonCh;
            statsResults.WithStimChInRegion.(regionNames{iRegion}).([cfgStats.legLabel{pairComps(iComp,:)}]).nCommonPairs =length(varibilityThisRegionComp2);
...
for iRegion=1:length(regionNames)
    ...
    for iComp=1:size(pairComps,1)
        ...
        plot(relativeSNR{iComp,iRegion},'.','MarkerSize',20);
        line([1 length(relativeSNR{iComp,iRegion})],[0 0],'Color', 'k','LineWidth',3)
        line([1 length(relativeSNR{iComp,iRegion})],[median(relativeSNR{iComp,iRegion}) median(relativeSNR{iComp,iRegion})],'Color', 'r','LineWidth',3)
        ylim([-1 1])
        xlim([0 length(relativeSNR{iComp,iRegion})+1])
        grid on;
        legLabel= strcat(cfgStats.legLabel(pairComps(iComp,:)),[' (', num2str(nPairsPerStatePerRegion(pairComps(iComp,1),iRegion)),')']);
        xticklabels({})

        title([legLabel{:}]);
        % boxplot of same data
        subplot(1,2*size(pairComps,1),iComp*2)
        boxplot(relativeSNR{iComp,iRegion})
        ylim([-1 1])
        yticklabels({})
        xticklabels([regionNames{iRegion}, ' ', num2str([pairComps(iComp,:)])])
    end
    suptitle(['Rel SNR ', regionNames{iRegion}])
    %Save figure
    ...
    relVarSleep = relativeSNR{indCompSleepWakeEMU,iRegion};
    relVarAnesthesia = relativeSNR{indCompAnesthesiaWakeOR,iRegion};
...
for iRelState=1:size(pairComps,1)
    ...
    for iCompRegion=1:size(pairRegionsComp,1)
        ...
        relativeVarRegion1 = relativeSNR{iRelState, pairRegionsComp(iCompRegion,1)};
        relativeVarRegion2 = relativeSNR{iRelState, pairRegionsComp(iCompRegion,2)};
...
%% Save info in MAT file
    [filepath,name,ext] = fileparts(cfgStats.xlsFileName);
    matFileNameResults = [filepath,filesep,'SNRPerRegion_',name,'.mat'] ;

save([matFileNameResults], 'regionNames', 'stimSitesPerStatePerRegion', 'cfgStats','pooledSNRFileName',...
    'SNRPerStatePerRegion','logSNRPerStatePerRegion','normSNRPerStatePerRegion', 'nTrialPerChPerStatePerRegion', ...
    'relativeSNR', 'pairComps', 'statsResults', ...
