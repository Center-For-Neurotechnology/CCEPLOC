function summaryOfAnesthesiaCCEPFeatures(dirPooledResults, pNames, posFixDir,whatToUseRespCh, anatRegionType, whichRecChannels)
...
% Resp Channels
dirRespFeatChPooled = [dirPooledResults,filesep,'Feat',whichRecChannels,anatRegionType];
respFeatChFileName = [dirRespFeatChPooled,filesep,'FeatRespCh','PerRegion_','Feat',whichRecChannels,'_',anatRegionType,'_',num2str(nPatients),'pat','.mat'];

% Output
fileNameSummary = [dirPooledResults,filesep,'summaryStimInfo_','Features_',whatToUseRespCh,'_',whichRecChannels,'_',anatRegionType,'_',num2str(nPatients),'pat_',date];
...
for iFeat =1:nFeatures
   ...
   for iRegion=1:nRegions
        if isfield(statsResults.RelFeatSleepAnesth,regionNames{iRegion}) && length(statsResults.RelFeatSleepAnesth.(regionNames{iRegion}).commonCh) >1
    ...
    for iRegion=1:nRegions
        if isfield(statsResults.RelFeatSleepAnesthNONPAIRED,regionNames{iRegion})
            dataToSave =  {regionNames{iRegion}, statsResults.RelFeatSleepAnesthNONPAIRED.(regionNames{iRegion}).(featureNames{iFeat}).pVal,...
                ...
                length(statsResults.RelFeatSleepAnesthNONPAIRED.(regionNames{iRegion}).(featureNames{iFeat}).chNamesEMU),...
                ...
                length(statsResults.RelFeatSleepAnesthNONPAIRED.(regionNames{iRegion}).(featureNames{iFeat}).chNamesOR)};
            disp(dataToSave)
            % m4Save
    ...
    for iComp=1:length(relCompStates)
        ...
        for iRegComp=1:length(regComp)
            dataToSave =  {relState,regionNamesToCompare{regComp(iRegComp,1)},regionNamesToCompare{regComp(iRegComp,2)},...
                ...
                length(statsResults.RelFeatRegionsNONPAIRED.(relState).([regionNamesToCompare{regComp(iRegComp,:)}]).(featureNames{iFeat}).chNamesRegion1),...
                length(statsResults.RelFeatRegionsNONPAIRED.(relState).([regionNamesToCompare{regComp(iRegComp,:)}]).(featureNames{iFeat}).chNamesRegion2)};
            disp(dataToSave)
            % m4Save
       ...
       for iRegion=1:nRegions
            if isfield(statsResults.FeaturesRespPerRegion, regionNames{iRegion}) && isfield(statsResults.FeaturesRespPerRegion.(regionNames{iRegion}),compNames{iComp}) && isfield(statsResults.FeaturesRespPerRegion.(regionNames{iRegion}).(compNames{iComp}),featureNames{iFeat})
                dataToSave =  {regionNames{iRegion}, statsResults.FeaturesRespPerRegion.(regionNames{iRegion}).(compNames{iComp}).(featureNames{iFeat}).pVal,...
                    ...
                    length(statsResults.FeaturesRespPerRegion.(regionNames{iRegion}).(compNames{iComp}).(featureNames{iFeat}).commonCh)};
                disp(dataToSave)
                % m4Save
                m4Save(iRow +iRegion, 1:nCols) = dataToSave;
            end
